...
namespace AdventOfCode2019
{
    public static class Day3
    {
        ...
        private static void GetWireCoordinates(List<string> input, HashSet<Coordinate> wire1Coordinates, IDictionary<Coordinate, int> coordinateSteps)
        {
            Coordinate currentPos = new Coordinate(0, 0);
            int totalDistance = 1;
            foreach (string dirString in input)
            {
                int distance = int.Parse(dirString.Substring(1));
                char d = dirString[0];
                Coordinate directionCoord;
                switch (d)
                {
                    case 'U':
                        directionCoord = new Coordinate(-1, 0);
                        break;
                    case 'R':
                        directionCoord = new Coordinate(0, 1);
                        break;
                    case 'D':
                        directionCoord = new Coordinate(1, 0);
                        break;
                    case 'L':
                        ...
                        break;
                    default:
                        throw new Exception($"bad direction {d}");
                }
                while (distance > 0)
                {
                    currentPos = directionCoord + currentPos;
                    wire1Coordinates.Add(currentPos);
                    if(!coordinateSteps.ContainsKey(currentPos)) coordinateSteps.Add(currentPos, totalDistance);
                    totalDistance++;
                    distance--;
                }
            }
